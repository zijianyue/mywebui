								...
								{#if message.info}
									<Tooltip
										content={message.info.openai
											? `输入的tokens: ${message.info.prompt_tokens ?? 'N/A'}<br/>
													输出的tokens: ${message.info.completion_tokens ?? 'N/A'}<br/>
													总计: ${message.info.total_tokens ?? 'N/A'}`
											: `response_token/s: ${
													`${
														Math.round(
		            ...
		            approximate_total: ${approximateToHumanReadable(message.info.total_duration ?? 0)}`}
								...
								{#if !readOnly}
									...
									{#if isLastMessage}
										...
										<Tooltip content={$i18n.t('回复内容的中英互译')} placement="bottom">
											<button
												type="button"
												class="{isLastMessage
													   ? 'visible'
