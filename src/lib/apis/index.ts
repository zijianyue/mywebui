...
export const getModels = async (token: string = '') => {
	...
	models = models
		.filter((model) => {
			...
			// 临时处理
			const cond4Gemma27b = !(model.id === 'google/gemma-2-27b-it' && model.urlIdx === 2);
...
export const generateSuggestQuestions = async (
	token: string = '',
	model: string,
	messages: object[],
	chat_id?: string
	...
	const res = await fetch(`${WEBUI_BASE_URL}/api/task/suggestQuestions/completions`, {
		method: 'POST',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
	...
	return res?.choices[0]?.message?.content ?? 'no suggest';
};
